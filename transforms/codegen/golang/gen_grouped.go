...
func (g *Generator) outputEnum(eName *ast.Identifier, eValue ast.Node) gast.Expr {
	// Enum type
	name := g.transpileIdentifier(eName)
	decl := &gast.GenDecl{
		Tok: gtoken.TYPE,
		Specs: []gast.Spec{
			&gast.TypeSpec{
		...
		for _, entry := range group.Entries {
			...
			case *ast.Entry:
				spec = &gast.ValueSpec{
					...
					Type: g.formatIdentifier(g.currentGroupName),
					Values: []gast.Expr{
						&gast.BasicLit{
							Kind:  gtoken.INT,
...
func (g *Generator) transpileArray(arr *ast.Array) (*structure, error) {
	...
	return newStructure(&gast.StructType{Fields: fl2.node.(*gast.FieldList)}), nil
...
func (g *Generator) transpileEntry(entry *ast.Entry) (*structure, error) {
	...
	tag := g.combineTags(
		[]string{
			g.tagPartWithLabel("codec", entry.Name.Name),
		},
	)
	field := &gast.Field{
...
func (g *Generator) transpileTypeChoice(c *ast.TypeChoice) (*structure, error) {
	...
	fmt.Printf(" ----- %T %s %T %s", c, c, c.First, c.First)
